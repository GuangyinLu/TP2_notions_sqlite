...
def LireEnregistrement(nomTable, **arg):
    ...
    if len(arg) == 0:
        ...
        MaRequeteAfficher = "SELECT * FROM {0} WHERE {1} = '{2}';".format(nomTable, list(arg)[0], list(arg.values())[0])
    elif len(arg) ==2:
        MaRequeteAfficher = "SELECT * FROM {0} WHERE {1} = '{2}' AND {3} = '{4}';".format(nomTable, list(arg)[0], list(arg.values())[0], list(arg)[1], list(arg.values())[1])
    conn = sqlite3.connect("tp2.db")
    cur = conn.cursor()
